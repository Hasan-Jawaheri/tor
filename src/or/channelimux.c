...
#define IMUXCIRC_TRAFFIC_TYPE(imuxcirc) (!(imuxcirc->circ) ? (TRAFFIC_TYPE_UNKNOWN) : (/*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType))
...
static size_t channel_imux_num_bytes_queued_method(channel_t *chan);
static int
...
channel_imux_common_init(channel_imux_t *imuxchan)
{
  ...
  //Lamiaa : added missing initialization
  chan->num_bytes_queued = channel_imux_num_bytes_queued_method;
  chan->num_cells_writeable = channel_imux_num_cells_writeable_method;
  // end Lamiaa
...
channel_imux_num_bytes_queued_method(channel_t *chan)
{
  ...
  SMARTLIST_FOREACH_BEGIN(imuxchan->connections, channel_imux_connection_t *, c)
   {
	  total_queued_size+= connection_get_outbuf_len(TO_CONN(c->conn));
   }
...
#endif
//log_info(LD_CHANNEL,"Lamiaa: outbuf_len = %d &  cell_network_size = %d & number of cells = %d",outbuf_len,cell_network_size,(int)n );
  return (int)n;
}
...
//    log_warn(LD_BUG, "Lamiaa: conn %p:  creating connection %d, move to CHANNEL_STATE_ERROR", imuxchan->newconn, i + 1);
...
channel_imux_close_connection(channel_imux_t *imuxchan, channel_imux_connection_t *imuxconn)
{
  ...
  if(!imuxconn->marked_for_close && imuxconn->conn->base_.state == OR_CONN_STATE_OPEN) {
...
//log_info(LD_BUG,"Lamiaa Max # of Connections = %d",channel_max_conns);
...
channel_imux_close_method(channel_t *chan)
{
  tor_assert(chan);
  channel_imux_t *imuxchan = BASE_CHAN_TO_IMUX(chan);
  //log_info(LD_BUG," Lamiaa: closing client channel : %d",chan->is_client );
...
#define MAX_DESCR_LEN 32
  ...
  addr_str = tor_addr_to_str_dup(&(imuxchan->addr));
...
channel_imux_get_single_web_connection(channel_imux_t *imuxchan, channel_imux_circuit_t *imuxcirc)
{
  ...
  if(imuxcirc->circ && /*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType == TRAFFIC_TYPE_BULK && imuxchan->bulk_connection)
  {
    imuxcirc->writeconn = imuxchan->bulk_connection;
    int traffic_type = -1;
    if(imuxcirc->circ) {
      traffic_type = /*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType;
    }
  ...
  if(imuxcirc->circ && /*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType != TRAFFIC_TYPE_WEB)
    return channel_imux_get_next_open_connection(imuxchan);
...
channel_imux_get_write_connection(channel_imux_t *imuxchan, channel_imux_circuit_t *imuxcirc, uint8_t command)
{
  ...
  if(get_options()->IMUXSeparateBulkConnection && imuxcirc && imuxcirc->circ && 
		  /*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType == TRAFFIC_TYPE_BULK && imuxchan->bulk_connection) {
    return imuxchan->bulk_connection;
  }

  if(get_options()->IMUXSeparateWebConnection && imuxcirc && imuxcirc->circ && 
		  /*imuxcirc->circ->traffic_type*/get_options()->IMUXTrafficType == TRAFFIC_TYPE_WEB && imuxchan->web_connection) {
                                     ...
                                     circuit_t *circ, packed_cell_t *packed_cell)
{
  ...
  connection_buf_add(packed_cell->body, cell_network_size, TO_CONN(conn));
  if(imuxconn)
     channel_imux_update_connection_ewma(imuxchan, imuxconn);
...
channel_imux_find_imux_connection(channel_t *chan, or_connection_t *conn)
{
  ...
  channel_imux_t *imuxchan = BASE_CHAN_TO_IMUX(chan);
//  log_info(LD_BUG, "Lamiaa--: Finding connection conn %d in imuxconn->conn %p in list length = %d \n", imuxchan->connections->capacity, conn,smartlist_len(imuxchan->connections));
...
//  log_info(LD_BUG, "Lamiaa--channel %p: connection %p going from state %d -> %d [chan state %d]", imuxchan, conn, old_state, state, chan->state);
...
channel_imux_flush_circ_queue(channel_imux_circuit_t *circ, or_connection_t *conn)
{
  while(smartlist_len(circ->cell_queue)) {
    ...
    smartlist_pqueue_pop(circ->cell_queue, channel_imux_compare_cells,
        offsetof(channel_imux_cell_t, minheap_idx));
  }
}
  ...
  // check if cell CELL_CLOSING_CONN so we can close this connection */
   if(cell->command == CELL_CLOSING_CONN) {
    ...
    if(cell->command == CELL_CLOSING_CHAN) {
	  //log_info(LD_BUG," Lamiaa: cell command = %d", cell->command);
  ...
  if(cell->sequence == imuxcirc->next_sequence) {
    ...
    if(imuxcirc->next_sequence == 1) {
        ...
        smartlist_pqueue_add(imuxcirc->cell_queue, channel_imux_compare_cells,
                offsetof(channel_imux_cell_t, minheap_idx), chan_cell);
    }
  }
}
...
static int channel_imux_get_chan_max_connections(channel_imux_t *imuxchan)
{
  ...
  {
//	  log_info (LD_BUG,"Lamiaa circuit traffic type = %d & n_circ_id =%d",circ->traffic_type,circ->n_circ_id );
    if(/*circ->traffic_type*/ get_options()->IMUXTrafficType == TRAFFIC_TYPE_WEB ||
...
//  }
  ...
  SMARTLIST_FOREACH_BEGIN(imuxchan->circuits, channel_imux_circuit_t *, imuxcirc) 
  {
    int traffic_type = get_options()->IMUXTrafficType;//IMUXCIRC_TRAFFIC_TYPE(imuxcirc);
...
channel_imux_remove_connection(channel_t *chan, or_connection_t *conn)
{
  ...
  if(smartlist_len(imuxchan->connections) == 0 && !chan->has_queued_writes(chan)) {
