...
static size_t channel_dual_num_bytes_queued_method(channel_t *chan);
static int
...
channel_dual_common_init(channel_dual_t *dualchan)
{
  ...
  chan->free_fn = channel_dual_free_method;
  ...
  //Lamiaa : added missing initialization
   chan->num_bytes_queued = channel_dual_num_bytes_queued_method;
   chan->num_cells_writeable = channel_dual_num_cells_writeable_method;
   // end Lamiaa
...
channel_dual_num_bytes_queued_method(channel_t *chan)
{
  ...
  SMARTLIST_FOREACH_BEGIN(imuxchan->connections, or_connection_t *, c)
   {
	  total_queued_size+= connection_get_outbuf_len(TO_CONN(c));
   }
...
channel_dual_num_cells_writeable_method(channel_t *chan)
{
  ...
  tor_assert(dualchan);
  tor_assert(dualchan->connections);

  SMARTLIST_FOREACH_BEGIN(dualchan->connections, or_connection_t *, c)
     {
	  cell_network_size += get_cell_network_size(c->wide_circ_ids);

	  outbuf_len+= connection_get_outbuf_len(TO_CONN(c));
     }
     SMARTLIST_FOREACH_END(c);
                    ...
                    const char *id_digest, const ed25519_public_key_t *ed_id)
{
  ...
  for(i = 0; i < NUMCONNECTIONS; i++) {
    ...
    conn = connection_or_connect(addr, port, id_digest, ed_id, chan);
...
#define MAX_DESCR_LEN 32
  ...
  tor_assert(dualchan);
  if(smartlist_len(dualchan->connections) > 0)
  tor_assert(smartlist_len(dualchan->connections));
...
channel_dual_flush_circ_queue(channel_dual_circuit_t *circ, or_connection_t *conn)
{
	while(smartlist_len(circ->cell_queue)) {
		...
		smartlist_pqueue_pop(circ->cell_queue, channel_dual_compare_cells,
				offsetof(channel_dual_cell_t, minheap_idx));
	}
}
  ...
  if(cell->sequence == dualcirc->next_sequence) {
    ...
    channel_dual_flush_circ_queue(dualcirc, conn);
      ...
      smartlist_pqueue_add(dualcirc->cell_queue, channel_dual_compare_cells,
              offsetof(channel_dual_cell_t, minheap_idx), chan_cell);
  }
}
...
channel_dual_remove_connection(channel_t *chan, or_connection_t *conn)
{
	...
	newconn = connection_or_connect(&(dualchan->addr), dualchan->port, dualchan->id_digest, &chan->ed25519_identity, chan);
